...
policyVarDecl (VarDecl _ (VarId _ i@(Ident _ rawI)) 
    ...
    = vDecl (voidAnn i) $ newStatic "Policy" (map (clauseToExp . addHeadVar) cs)
...
compileStmt (Open t (Lock _ lN aN )) = ExpStmt () $
         callStatic "LockState" "open"
         [newStatic "Lock" ( ExpName () (voidAnn lN) : 
         (map (\a -> newStatic "Actor" [a])
         (map (compileExp . (\aname -> case aname of
                                          ActorName _ x -> ExpName t x
                                          ActorTypeVar _ _ i -> ExpName t (Name t EName Nothing i)
                             )) aN)))]
compileStmt (Close t (Lock _ lN aN )) = ExpStmt () $
         callStatic "LockState" "close"
         [newStatic "Lock" ( ExpName () (voidAnn lN) : 
         (map (\a -> newStatic "Actor" [a])
         (map (compileExp . (\aname -> case aname of
                                          ActorName _ x -> ExpName t x
...
compileMethodInv mi = case mi of
  MethodCallOrLockQuery _ n@(Name _ LName _ _) as ->
    MethodCallOrLockQuery () 
      (Name () MName (Just $ mkPkgTypeName "LockState") (Ident () (B.pack "isOpen")))
      [newStatic "Lock" (ExpName () (voidAnn n) :
        map (\a -> newStatic "Actor" [a]) (map compileExp as)
      )]
...
compilePolicyExp (PolicyLit _ cs) = newStatic "Policy" (map (clauseToExp . addHeadVar) cs)
...
-- Add the missing quantified variable from the head (for policy clauses, do not
-- call for lock properties).
addHeadVar :: Clause T -> Clause T
addHeadVar (Clause a vars h@(ClauseDeclHead _ v) body) =
  Clause a (v : vars) h body
addHeadVar (Clause a vars h@(ClauseVarHead _ _) body) =
  Clause a vars h body

clauseToExp :: Clause T -> Exp ()
clauseToExp (Clause _ vars h body) = 
    let vs = nub [ a | Var _ a <- universeBi (clauseHeadToActor $ voidAnn h) 
        ...
        sorts = newArrayInit "Class" (map (\(ClauseVarDecl a r _) -> classField r ) vars)
     in newStatic "Clause" (sorts : exps)
...
atomToExp vs (Atom _ n acts) = 
    newStatic "Atom" (ExpName () (voidAnn n): map (actorToExp vs) acts)

actorToExp :: [(Ident (), Int)] -> Actor T -> Exp ()
actorToExp _vs (Actor _ (ActorName _ n)) = newStatic "Actor" [ExpName () $ voidAnn n]
...
actorToExp vs (Var _ i) = 
    let res = lookup (voidAnn i) vs
     ...
     in newStatic "Variable" [Lit () $ Int () k]
      ...
      LockDecl _ ms i@(Ident _ rawI) pars mLProps ->
              let -- Properties defined in modifiers
                  ...
                  lpExps = maybe [] 
                             ...
                             mLProps
                  lockE = newStatic "LockID" $ (Lit () $ String () $ B.unpack rawI) :
                             (map (\r -> classField r) pars)
...
lockPropToExp _i@(Ident _ rawI) (LClause _ vars h body) =
    let vs = nub [ a | Var _ a <- universeBi (map voidAnn (h:body)) ] `zip` [0..] -- Substs
        exps = atomToExp vs h : map (atomToExp vs) body
        sorts = newArrayInit "Class" (map (\(ClauseVarDecl a r _) -> classField r ) vars)
        lockprop = newStatic "LockProperty" (sorts : exps)
    in callStatic "GlobalPolicy" "addProperty" [lockprop]
       --call [B.unpack rawI,"addClause"] exps
...
getRefName :: RefType a -> Name ()
getRefName (ClassRefType _ ctype) = case ctype of
    (ClassType _ n _) -> removeAnnotation n
getRefName (TypeVariable _ann id) = Name () TName Nothing (removeAnnotation id)
getRefName (ArrayType _ _ _) = error "array types dont get compiled as lock arguments"

classField :: RefType a -> Exp ()
classField r =  FieldAccess () (PrimaryFieldAccess () (ExpName () (getRefName r)) (Ident () (B.pack "class")))

newStatic :: String -> [Exp ()] -> Exp ()
newStatic typ args = 
    InstanceCreation () [] (ClassType () (mkPkgTypeName typ) []) args Nothing

newArrayInit :: String -> [Exp ()] -> Exp ()
newArrayInit typ exp =
    ArrayCreateInit () (RefType () (ClassRefType () (ClassType () (Name () TName Nothing (Ident () (B.pack typ))) [Wildcard () Nothing]))) [Nothing] (ArrayInit () (map (InitExp ()) exp))
...
pkgPrefix :: Name ()
pkgPrefix = mkUniformName const PName $ map (Ident () . B.pack) ["se","chalmers","paragon","runtime"]
